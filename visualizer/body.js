...
// URL parameter will be handled after the change event listener is attached
// handle UI changes
...
window.onload = () => {
    ...
    // 2️⃣ Sync dropdown with URL
    if (algoSelect && info[algoFromURL]) {
        algoSelect.value = algoFromURL;

        // 3️⃣ Trigger change (THIS handles everything)
        algoSelect.dispatchEvent(new Event('change', { bubbles: true }));
    }
     toggleInputs(algoFromURL); 

};
// 🔥 AUTO LOAD ALGORITHM FROM HOMEPAGE - Run immediately (DOMContentLoaded already fired by script load time)
...
// Handle URL parameter now that change event listener is attached
...
arr = arrRaw.map(Number);
   ...
   if(["firstfit","bestfit","worstfit"].includes(algo)){
    ...
    const queuePanel = readyQueueElement ? readyQueueElement.parentElement : null;

    if (queuePanel && ["fcfs","sjf","rr"].includes(algo)) {
        queuePanel.style.display = "block";
    } else if (queuePanel) {
        queuePanel.style.display = "none";
    }
...
function renderAlgorithm(algo) {
    ...
    // update the left-side detail panel
    updateDetail(algo);

    // ✅ FIX: also update the top info panel immediately when algo changes
    if (info[algo]) {
        const titleEl = document.getElementById("algoTitle");
        if (titleEl) titleEl.textContent = info[algo].name;
        if (explanationPanel) explanationPanel.textContent = info[algo].description;
        if (complexityPanel) complexityPanel.textContent = info[algo].complexity;
    }
}
...
function renderDisk(requests, head){
    ...
    // 🔥 SORT TRACK
    const allPoints = [...requests, head]
        .filter((v, i, a) => a.indexOf(v) === i)
        .sort((a, b) => a - b);

    // 🔥 RENDER NODES
    allPoints.forEach(val => {
    ...
    headLabel.textContent = "Start: " + head;

    container.insertBefore(headLabel, track);

    // 🔥 DISK HEAD
    const headNode = document.createElement("div");
    headNode.classList.add("disk-head");
    headNode.id = "diskHead";
    headNode.textContent = "H";
    ...
    const nodes = track.querySelectorAll(".disk-node");
...
function toggleInputs(algo){
    ...
    document.getElementById("headInput").style.display = "none";
    document.getElementById("quantumInput").style.display = "none";

    // 🟢 SORTING / SEARCHING
    if(["bubble","selection","insertion","merge","quick","heap","linear","binary"].includes(algo)){
        document.getElementById("arrayInput").style.display = "inline-block";
    }

    // 🟢 CGIP
    // PAGE REPLACEMENT
else if(["fifo","lru","optimal"].includes(algo)){
    document.getElementById("arrayInput").style.display = "inline-block";
    document.getElementById("frameInput").style.display = "inline-block";
}
   // 🟢 DDA & Bresenham (4 inputs)
else if(["dda","bresenham"].includes(algo)){
    ...
    document.getElementById("y2").style.display = "inline-block";
}

// 🟢 Circle (ONLY 3 inputs)
else if(algo === "circle"){
    document.getElementById("x1").style.display = "inline-block";
    document.getElementById("y1").style.display = "inline-block";
    document.getElementById("x2").style.display = "inline-block";
    ...
    // 🟢 MEMORY (FIRST FIT etc.)
    else if(["firstfit","bestfit","worstfit"].includes(algo)){
        document.getElementById("blockInput").style.display = "inline-block";
        document.getElementById("processInput").style.display = "inline-block";
    }

    // 🟢 DISK
    else if(["diskfcfs","sstf","scan"].includes(algo)){
        document.getElementById("arrayInput").style.display = "inline-block";
        document.getElementById("headInput").style.display = "inline-block";
    }

    // 🟢 SCHEDULING
    else if(["fcfs","sjf","rr"].includes(algo)){
        document.getElementById("arrayInput").style.display = "inline-block";

        if(algo === "rr"){
            document.getElementById("quantumInput").style.display = "inline-block";
        }
    }
}
...
function renderGanttForScheduling(steps) {
    ...
    const ganttWidth = gantt.scrollWidth || 900;
...
if(["diskfcfs","sstf","scan"].includes(algo)){
    ...
    nodes.forEach(n => {
        ...
        if(Number(n.textContent) === step.to){
            ...
            // 🔥 move head (centered)
            head.style.left =
                pos + n.offsetWidth / 2 - head.offsetWidth / 2 + "px";

            // 🔥 keep head label constant
            head.textContent = "H";

            // ✅ mark as visited (permanent)
            n.classList.add("visited");

            // ✅ highlight current (temporary)
...
//// LINE & CIRCLE DRAWING
/*if(algo === "dda" && step.type === "complete"){
    console.log("DDA completed");
}

if(["dda","bresenham","circle"].includes(algo)){

    // 🔥 skip non-visual steps but DON'T break loop
    if(!["plot","start","end"].includes(step.type)) {
        // still update step text panel
    ...
    // START
    if(step.type === "start"){
        pixel.classList.add("start","visited");
    }

    // END
    else if(step.type === "end"){
        pixel.classList.add("end","visited");
    }
...
const hasBars = bars.length > 0;
        ...
        // ================= UPDATE BARS FOR SORTING =================

    if (step.array && bars.length) {
    step.array.forEach((val, i) => {
        if (bars[i]) {
            bars[i].style.height = val * 4 + "px";
            bars[i].querySelector("span").textContent = val;
        }
    });
}
